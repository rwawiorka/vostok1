	...
	public class GeneratorsTester : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		#region Create Test Case
			...
			public static void CreateTestCase (Generator gen, TileData data)
			{
				...
				tester.output = CreateTestOutput(outgo, (dynamic)gen);
			}
			...
			public static ITestInput CreateTestInput<T> (IInlet<T> inlet, T product) where T: class
			{
			...
			public static ITestOutput CreateTestOutput<T> (GameObject go, IOutlet<T> outlet) where T: class
			{
		...
		#region Products Data Mockup
			...
			/// The products data that allows storing products per-inlet
			{
				...
				public new dynamic this[IOutlet<object> outlet]
				{
					get 
					{
				...
				public dynamic this[IInlet<object> inlet]
				{
					get 
					{
