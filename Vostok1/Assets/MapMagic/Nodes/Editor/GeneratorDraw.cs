...
//				using (Timer.Start("Footer"))
		...
		/// Internal move function for DragGenerator
		{
			...
			if (Event.current.control)
				moveTo = new Vector2(
					(int)(moveTo.x / 32 + 0.5f) * 32,
					(int)(moveTo.y / 32 - 0.5f) * 32 );
			...
			using (Cell.LinePx(24))
			{
				if (gen is IInlet<object> inletGen)
				{
					...
					if (menuAtt.nameWidth < nodeWidth-24 -10 -(gen is IOutlet<object> ? 10 : 0))
						using (Cell.RowPx(24)) Draw.Icon(menuAtt.icon);
				}
			...
			using (Cell.LinePx(portalHeight))
			{
				...
				using (Cell.LinePx(24))
				{
						if (portalEnter != null)
						{
							...
							using (Cell.RowPx(20)) Draw.Icon(genIcon);
		...
		public static void DrawGlobalVar<T> (ref T val, string label)
		{
		...
		#region Helpers
			...
			private static ValAttribute[] GetInletVals (Type genType)
			{
