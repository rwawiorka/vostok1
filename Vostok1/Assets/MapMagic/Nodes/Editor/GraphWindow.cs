...
namespace MapMagic.Nodes.GUI
{
	...
	public class GraphWindow : EditorWindow
	{
		...
		private bool disableAddRemoveButton = false;  //turning off Add/Remove on opening popup with it, and re-enabling once the graph window is focused again
		...
		public void OnEnable () 
		{
			...
			ReFocus(); //focusing after script re-compile
		}
		...
		private void OnGUI()
		{
			...
			graph.guiScrollPivot = graphUI.scrollZoom.GetWindowCenter(position.size); 
			...
			{using (Timer.Start("DrawGraph"))
		{
				...
				//clearing temporary rects
				/*if (!UI.current.layout)
				{
					DrawGenerator.activeGeneratorCells.Clear();
					DrawGenerator.activeInletCells.Clear();
					DrawGenerator.activeOutletCells.Clear();
				}

				//right-click
				//if (Event.current.type == EventType.ContextClick) //this one works only on mouse up, which has a wrong position because of a toolbar
				if (!UI.current.layout &&  Event.current.type == EventType.MouseDown  &&  Event.current.button == 1)
					GUI.GraphPopupMenus.DrawRightClickItems(graphUI.mousePos, graph);

				//storing graph pivot to focus it on load
				graph.guiScrollPivot = graphUI.scrollZoom.GetWindowCenter(position); */
		}}
				...
				using (Cell.LinePx(toolbarSize))
		{
			...
			using (Cell.RowPx(20))
			{
				if (Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/FolderTree"), visible:false))
			...
			{
				if (parentGraphs != null  &&  parentGraphs.Count != 0  && 
					Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/FolderUp"), visible:false))
				{
				...
				using (Cell.LinePx(toolbarSize-1)) //-1 to leave underscore under gauge
			{
				if (mapMagic != null)
				{
					...
					using (Cell.RowPx(20))
						if (Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/RefreshAll"), visible:false))
						{
					...
					using (Cell.RowPx(20))
						if (Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/Refresh"), visible:false))
						{
							GraphWindow.current.mapMagic.StartGenerate();
						}
			...
			//focus
			using (Cell.RowPx(20))
				if (Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/FocusSmall"), visible:false))
				{
					graph.guiScrollPivot = GetNodesCenter(graph);
					ReFocus();
				}
			...
			using (Cell.RowPx(20))
			{
				...
				{
					if (Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/ZoomSmallPlus"), visible:false))
						graphUI.scrollZoom.Zoom(1f, position.size/2);
				}
				else
				{
					if (Draw.Button(null, icon:UI.current.textures.GetTexture("DPUI/Icons/ZoomSmallMinus"), visible:false))
		...
		{
			Graph winGraph = graph;
			if (winGraph == null) return; 
			graphUI.scrollZoom.FocusWindowOn(winGraph.guiScrollPivot, position.size);
		}
		...
		private void DragDrawAddRemove ()
		{
			...
			{
				GeneratorRightClick.RemoveGenerator(releasedGen, graph, selected);
				GraphWindow.RefreshMapMagic();
			}
		...
		#region Showing Window
			...
			/// Opens the graph window. But it should be created and graph assigned first.
			{
				...
				window.ReFocus(); //focusing after window has shown (since it needs window size)
			}
			...
			public static bool ShowEditor (int instanceID, int line)
			{
				...
				{ 
					if (UI.current != null) UI.current.DrawAfter( new Action( ()=>Debug.Log(graph) ) ); //if opened via graph while drawing it - opening after draw
					else Show(graph); 
					return true; 
				}
