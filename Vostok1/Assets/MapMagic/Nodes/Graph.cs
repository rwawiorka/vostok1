...
namespace MapMagic.Nodes
{
	...
	public class Graph : ScriptableObject , ISerializationCallbackReceiver
	{
		...
		//caches
		//[NonSerialized] Dictionary<IOutlet<object>, IInlet<object>[]> cachedBackwardLinks = null;
		//[NonSerialized] Dictionary<Guid, Generator> cachedGuidLut = null;
		//Is created when first time needed, and resets on any link change

		public Vector2 guiScrollPivot;  //the last viewed point at the window center to focus on opening graph
		...
		#region Iterating Nodes
			...
			public Generator GetGenerator (Guid guid)
			{
				bool findByGuid (Generator g) => g.guid==guid;
				return GetGenerator(findByGuid);
			}
