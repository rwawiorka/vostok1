...
namespace Den.Tools.GUI
{
	public class UI
	{
		...
		#region Draw
			...
			public void DrawScrollable (Action action, bool usePadding=false)
			...
			/// usePadding offsets 3 pixels from the window borders
			{
				...
				Draw(action, usePadding:usePadding);

				EditorGUILayout.EndScrollView();
			}


			public void Draw (Action drawAction, bool usePadding=false, bool inSameRect=false)
			{
				...
				if (!inSameRect) 
					rect = GUILayoutUtility.GetRect(new GUIContent(), usePadding? EditorStyles.helpBox : GUIStyle.none);
				else 
