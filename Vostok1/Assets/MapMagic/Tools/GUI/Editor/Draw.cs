	...
	public static class Draw
	{
		...
		#region Fast Fields
			...
			public static float Field (float val) 
			{
				...
				Rect rect = Cell.current.GetRect(UI.current.scrollZoom, padding:fieldPadding);
				float newVal = EditorGUI.FloatField(rect, val, style:UI.current.styles.field);  
			...
			public static float Field (float val, GUIStyle style) 
			{
				...
				Rect rect = Cell.current.GetRect(UI.current.scrollZoom, padding:fieldPadding);
				float newVal = EditorGUI.FloatField(rect, val, style:style);
			...
			public static int Field (int val) 
			{
				...
				int newVal = (int)EditorGUI.FloatField(rect, val, style:UI.current.styles.field);
		...
		#region Other Elements
			...
			public static bool Button (Texture2D icon, bool visible=true, MouseCursor cursor=0) 
			{
			...
			public static bool Button (string label, Texture2D icon, int iconWidth=20, bool visible=true, MouseCursor cursor=0) 
			{
				bool pressed = Button(visible:visible, cursor:cursor);
				using (Cell.RowPx(iconWidth)) Icon(icon);
				using (Cell.Row) Label(label);
				return pressed;
			}
			...
			public static void Icon (Texture2D icon, Vector2 center, Color color = new Color())
			/// Draws an icon in the given coord in native resolution (for zoom 1). Independent from cell
			{
				...
				Rect rect = new Rect(center.x - icon.width/2f, center.y - icon.height/2f, icon.width, icon.height);
			...
			public static void Icon  (Texture2D icon, Color color = new Color())
			/// Draws an icon in the center of cell in native resolution (for zoom 1)
			{
				if (UI.current.layout) return;
				if (UI.current.optimizeElements && !UI.current.IsInWindow()) return;

				Icon(icon, Cell.current.InternalCenter, color);
			}
			...
			public static void TypeSelector<T> (ref T obj, string label, ref Type[] allTypes, ref string[] allNames)
			{
				if (allTypes == null)
				{
					allTypes = typeof(T).Subtypes();
			...
			/// Draws grid in window-related rect. Moves with zoom. Useful for backgrounds
			{
				...
				Vector2 dispOffset = new Vector2(-UI.current.scrollZoom.scroll.x + displayRect.x, UI.current.scrollZoom.scroll.y-displayRect.height - displayRect.y+1); //-1 makes the line pass through 0-1 pixel
			...
			/// Draws grid in window-related rect
			{
				...
				gridMat.SetFloat("_CellSizeX", cellSize.x);
				gridMat.SetFloat("_CellSizeY", cellSize.y);
				gridMat.SetFloat("_CellOffsetX", cellOffset.x); 
				gridMat.SetFloat("_CellOffsetY", cellOffset.y);

				gridMat.SetFloat("_LineOpacity", lineOpacity);
				gridMat.SetFloat("_BordersOpacity", bordersOpacity);

				gridMat.SetVector("_Rect", new Vector4(displayRect.x, displayRect.y, displayRect.size.x, displayRect.size.y));
		...
		#region Animation Curve
				...
				Color color = new Color())
			{
				...
				try
				{
					...
					if (UI.current.editorWindow != null  &&  UI.current.editorWindow.GetType() == curveWindowType)
					{
						AnimationCurve windowCurve = curveWindowType.GetProperty("curve").GetValue(UI.current.editorWindow, null) as AnimationCurve;
						if (windowCurve == src)
						{
