...
*/				
			...
			public static void Multiply (this Texture2D tex, Color color)
			{
				float cr = color.r; float cg = color.g; float cb = color.b; float ca = color.a;

				Color[] colors = tex.GetPixels();
				for (int i=0; i<colors.Length; i++)
				{
					float r = colors[i].r; float g = colors[i].g; float b=colors[i].b; float a=colors[i].a;
					colors[i] = new Color(
						r*cr, //(r*cr)*ca + r*(1-ca),
						g*cg, //(g*cg)*ca + g*(1-ca),
						b*cb, //(b*cb)*ca + b*(1-ca),
