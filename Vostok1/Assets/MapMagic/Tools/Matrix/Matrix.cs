		...
		#region Other
			...
			/// PaddedOnePixel works similarly to AA, but fills border pixels with full value (to create main tex for the mask)
			{
				int numSteps = Mathf.Max(
					Mathf.Abs((int)end.x - (int)start.x),  //NOT (int)(end-start)!
					Mathf.Abs((int)end.y - (int)start.y) );
				...
				int tmp = endInclusive ? 1 : 0;
				for (int s=0; s<numSteps+tmp; s++)
				{
					...
					int ix = (int)(float)fx;  //if (x<0) ix--;  
					int iz = (int)(float)fz;  //if (z<0) iz--;  
