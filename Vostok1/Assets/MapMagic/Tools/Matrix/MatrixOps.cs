		...
		#region Free Resize
			...
			/// Using temp array for downsizing both dimensions (tmp should be src size at least vertically)
			{
				...
				private static void DownsizeVertically (Matrix src, Matrix dst, Stripe srcStripe, Stripe dstStripe)
				{
					...
					{
						ReadRow(srcStripe, src, x, dst.rect.offset.z);
						ResampleStripeLinear(srcStripe, dstStripe);
						WriteRow(dstStripe, dst, x, dst.rect.offset.z);
					}
				}
				...
				private static void UpsizeHorizontally (Matrix src, Matrix dst, Stripe srcStripe, Stripe dstStripe)
				{
					...
					for (int z=max-1; z>=min; z--) //from end to start since in some cases we would be using the same array with different rects
					{
						ReadLine(srcStripe, src, src.rect.offset.x, z);
