...
//				ArrayTools.Add(ref markerRects, rect);
	...
	public class ExportPlugin : IPlugin
	{
		...
		public static void MatrixToTexture (Matrix matrix, ref Texture2D tex)
		{
			if (tex == null || tex.width != matrix.rect.size.x || tex.height != matrix.rect.size.z)
			{
				tex = new Texture2D(matrix.rect.size.x, matrix.rect.size.z, textureFormat:TextureFormat.RGB24, mipChain:false);
				tex.filterMode = FilterMode.Point;
			}

			matrix.ExportTextureRaw(tex);
		}


		public static void MatrixToTerrain (Matrix matrix, TerrainData terrainData)
		{
