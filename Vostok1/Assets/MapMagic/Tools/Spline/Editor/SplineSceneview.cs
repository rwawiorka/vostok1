...
namespace Den.Tools.Splines
{
	public static class SplineEditor
	{
		...
		#region Edit
			...
			public static void EditSplineSys (SplineSys sys, Matrix4x4 trs)
			/// Select, move, rotate, scale nodes (and draw selection/transform gizmos)
			{
				...
				if (eventCurrent.type == EventType.MouseUp  &&  eventCurrent.button == 0  &&  !eventCurrent.alt  &&  !SceneEdit.Select.isFrame)  //releasing mouse with no frame
				{
					...
					if (eventCurrent.shift) 
						AddNode(sys, eventCurrent.mousePosition);

					//removing
					else if (eventCurrent.control)
						RemoveNode(sys, eventCurrent.mousePosition);
				}
				...
				if (selectedKnobs.Count == 1)
				{
					...
					bool change = posChange || dirChange;
				...
				if (selectedKnobs.Count > 1)
				{
					...
					bool change = MoveSelectedNodes(knobs);

					if (change) //updating all selected nodes
					{
			...
			private static void AddNode (SplineSys sys, Vector2 mousePos)
			{
				...
				(Location location, float dist) = sys.GetClosest(DistFn);
				...
				if (dist < 10) //10 pixels from line on screen
				{
					...
					Line line = sys.lines[location.l];
					line.Split(location.s, location.p);
			...
			private static void RemoveNode (SplineSys sys, Vector2 mousePos)
			{
				int lineNum = 0;
				int nodeNum = 0;
