...
{ 
	...
	/// Line/segments numbers and segment percent to identify point on a spline
	{
		public int l;
		public int s;
		public float p;

		public Location (int l, int s, float p) { this.l=l; this.s=s; this.p=p; }
		public Location (int s, float p) { this.l=-1; this.s=s; this.p=p; }
		public Location (float p) { this.l=-1; this.s=-1; this.p=p; }
	}
	...
	public class SplineSys : ICloneable
	{
		...
		public Vector3 GetPoint (Location location) { return lines[location.l].segments[location.s].GetPoint(location.p); }
		public Vector3 GetPoint (int l, int s, float p) { return lines[l].segments[s].GetPoint(p); }
		...
		public (Location loc, float dist) GetClosest (Vector3 point, int initialApprox=10, int recursiveApprox=10) =>
			GetClosest(null, point, initialApprox, recursiveApprox);

		public (Location loc, float dist) GetClosest (Func<Vector3,Vector3,float> distanceFn, int initialApprox=10, int recursiveApprox=10) =>
			GetClosest(distanceFn, new Vector3(), initialApprox, recursiveApprox);

		public (Location loc, float dist) GetClosest (Func<Vector3,Vector3,float> distanceFn, Vector3 point, int initialApprox=10, int recursiveApprox=10)
		...
		/// If distFn defined using it instead of point
		{
			...
			for (int l=0; l<lines.Length; l++)
			{
				(Location curLoc, float curDist) = lines[l].GetClosest(distanceFn, point, initialApprox, recursiveApprox);
				if (curDist < minDist)
				{
					minDist = curDist;
					minLoc.s = curLoc.s;
