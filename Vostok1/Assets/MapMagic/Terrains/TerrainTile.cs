	...
	// TODO: when using Voxeland, use an area or a special VoxelandTile with the same interface
	{
		...
		#region ITile
			...
			public void Move (Coord newCoord, float newRemoteness)
			{
				...
				Vector3 size = mapMagic.tileSize;
			...
			public Terrain CreateTerrain (bool isDraft)
			{
				...
				terrainData.size = mapMagic.tileSize;
		...
		#region Threaded
			...
			public class StopTask : Den.Tools.Tasks.ThreadManager.Task
			{
			...
			/// Will run task no matter if previous task is running (draft-style)
			{
				if (det.task == null)
				{
					...
					det.task = new ThreadManager.Task() { 
						action = ()=>Generate(graph, this, det, det.stop), //note the graph captured
						priority = priority, 
						name = name + " " + coord };
				}
...
//				if (!stop.stop) graph.CheckClear(det.data, stop);
		...
		public void OnDrawGizmos_Tmp ()
		{
			...
			Vector3 center = coord.vector3 * mapMagic.tileSize.x + mapMagic.tileSize/2;
			Gizmos.DrawWireCube(center, mapMagic.tileSize);
