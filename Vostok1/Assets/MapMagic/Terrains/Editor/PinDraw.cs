...
namespace MapMagic.Core.GUI
{
	public static class PinDraw
	{
		...
		public static void DrawSceneGUI (MapMagicObject mapMagic, SelectionMode selectionMode)
		{
			...
			if (selectionMode!=SelectionMode.none  &&  !Event.current.alt)
			{
				...
				if (selectionMode == SelectionMode.pin)
				{
					...
					List<Coord> selectedCoords = SelectTiles(mapMagic.tileSize, false, tilesLut);
				...
				if (selectionMode == SelectionMode.pinLowres)
				{
					...
					List<Coord> selectedCoords = SelectTiles(mapMagic.tileSize, true, tilesLut);
					if (selectedCoords != null)
					{
		...
		#region Selecting
			...
			public static List<Coord> SelectTiles (Vector3 tileSize, bool dotted, Dictionary<Coord,TerrainTile> terrainsLut=null)
			...
			/// Could use only the terrainsLut, but that's not so intuitive
			{
				...
				List<Coord> framedCoords = GetTilesInFrame(Select.frameRect, tileSize);
				
				//displaying frame
				foreach (Coord coord in framedCoords)
					FrameDraw.DrawFrame(coord, tileSize, FrameDraw.pinColor, dotted, terrainsLut, FrameDraw.defaultZOffset*2);
			...
			private static List<Coord> GetTilesInFrame (Rect screenFrame, Vector3 tileSize)
			{
