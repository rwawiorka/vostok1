...
namespace MapMagic.Core.GUI
{
	public static class FrameDraw
	{
		...
		public static void DrawClock (TerrainTile tile)
		{
		...
		public static void DrawFrame (Coord coord, Vector3 tileSize, Color color, bool dotted, Dictionary<Coord,TerrainTile> terrainsLut=null,  float offset=defaultZOffset)
		/// Drawing a frame on terrain (if found in terrainsLut) or on the ground level
		{
			...
			//drawing empty
				...
				DrawEmptyFrame(coord.vector3.Mul(tileSize), new Vector3(tileSize.x,0,tileSize.z), color, dotted, offset);
		}


		public static void DrawEmptyFrame (Vector3 start, Vector3 size, Color color, bool dotted, float offset=defaultZOffset)
		{
		...
		public static void DrawTerrainFrame (Terrain terrain, Color color, bool dotted, float offset=defaultZOffset)
		{
			...
			polyLine.DrawLine(color, width, dotted ? terrain.terrainData.size.x/dotsPerSide : 0, offset:offset);
		}
