...
namespace MapMagic.Core 
{
	...
	{
		public static readonly SemVer version = new SemVer(2,0,0); 
		...
		/*public static MapMagicObject MapMagicUsingTile (TerrainTile tile)
		{
			int allCount = all.Count;
			for (int i=0; i<allCount; i++)
			{
				MapMagicObject mmo = all[i] as MapMagicObject;
				if (mmo==null) continue;

				if (mmo.tiles.Contains(tile.Coord 
			}
		}*/
		...
		public void Update () 
		{ 
			Profiler.BeginSample("Update Tiles");
			tiles.Update(tileSize, this, distsOnly: !isPlaying); //distsOnly: only updating distance priority in editor
			Profiler.EndSample();

			//Profiler.BeginSample("Update Coroutines");
