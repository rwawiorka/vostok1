		...
		public void DrawGUI ()
		{
			...
			using (Cell.LinePx(20))
			{
				...
				using (Cell.RowPx(22))
				{
					...
					if (Draw.Button(icon:openIcon, visible:false))
			...
			using (Cell.LinePx(30))
			{
				...
				using (Cell.RowPx(30)) Draw.Icon(UI.current.textures.GetTexture("DPUI/Icons/RefreshAll"));
			}
			...
			using (Cell.LinePx(22))
			{
				...
				using (Cell.RowPx(30)) Draw.Icon(UI.current.textures.GetTexture("DPUI/Icons/Refresh"));
			}
					...
					if (guiAbout)
					{
						using (Cell.Line)
						{
							...
							using (Cell.Row)
							{
								...
								using (Cell.LineStd) Draw.URL(" - Video Tutorials", url:"https://www.youtube.com/playlist?list=PL8fjbXLqBxvZb5yqXwp_bn4keyzyg5e0R");
								...
								using (Cell.LineStd) Draw.URL(" - Issues / Ideas", url:"https://gitlab.com/denispahunov/mapmagic/issues");
							}
						}
					}
		}
